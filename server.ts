...
app.post("/api/chat", authenticateToken, async (req: any, res: any) => {
  ...
  try {
    ...
    return res.status(edgeResponse.status).json({
      ...data,
      remaining: typeof data.remaining === "number" ? data.remaining : creditStatus.remaining,
    });
  } catch (error: any) {
    ...
    try {
      ...
      if (first !== -1 && last !== -1 && last > first) {
        ...
        challengeResult = {
          ...
          dimensions: { attention, clarity, desire, persuasion, action }
        };
      }
    } catch (e) {
...
// Dedicated Murgii AI Generation Endpoint forwarding to Supabase murgii-generate Edge Function
app.post("/api/murgii/generate", authenticateToken, async (req: any, res: any) => {
  ...
  try {
    ...
    if (finalChallengeResult?.shareSlug) {
      challengeStore.set(finalChallengeResult.shareSlug, {
