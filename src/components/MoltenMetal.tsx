...
export const MoltenMetal = ({
  ...
  useEffect(() => {
    ...
    if (u.uCoreSize) u.uCoreSize.value = Math.max(coreSize, 0.001);
    if (u.uSwirl) u.uSwirl.value = swirl;
    if (u.uFold) u.uFold.value = fold;
    if (u.uBlackPoint) u.uBlackPoint.value = blackPoint;
    if (u.uBrightness) u.uBrightness.value = brightness;
    if (u.uColorMode) u.uColorMode.value = colorModeToFloat(colorMode);
    if (u.uGrain) u.uGrain.value = grain ? 1 : 0;
    if (u.uGrainIntensity) u.uGrainIntensity.value = grainIntensity;
    if (u.uOpacity) u.uOpacity.value = opacity;
    if (u.uMouseStrength) u.uMouseStrength.value = mouseStrength;
  ...
  return (
    <div ref={containerRef} className={`molten-metal-container ${className}`.trim()}>
      {!hasWebGL && (
        <div className="absolute inset-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-fuchsia-900/40 via-purple-950/50 to-transparent animate-pulse" />
      )}
    </div>
  );
};
